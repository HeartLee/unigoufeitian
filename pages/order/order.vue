<template>
	...
	<scroll-view style="margin-top:80rpx;height: calc(100%-100rpx); background-color: #F5F5F5;" 
	class="scroll" id="scrollview" scroll-y="true" 
	 >
	 <!-- 列表 -->
	 <view class='listBox'  v-for="(item , index) in tabContent" :key="index" > 
	  ...
	  <view class='status' v-if="item.status==2" style=" margin-left: calc(100% - 340rpx)">
	      <button style="color:#fa3534; border: 1px solid #fa3534;" @click="gotodetail(item.id)" >查看详情</button>
	   <button  style="color:#fa3534; border: 1px solid #fa3534;" @click="uploadfile(item.id)">上传资料</button>
	  </view>
	  <view class='status' v-if="item.status==1" style=" margin-left: calc(100% - 510rpx)">
	    <button style="color:#666666; border: 1px solid #666666;" @click="cancelOrder(item.id)" >取消订单</button>
	   <button style="color:#fa3534; border: 1px solid #fa3534;" @click="gotodetail(item.id)">查看详情</button>
	   <button  style="color:#fa3534; border: 1px solid #fa3534;" @click="uploadfile(item.id)" >上传资料</button>
	  </view>
	  <view class='status' v-if="item.status==3" style=" margin-left: calc(100% - 340rpx)">
	   <button style="color:#fa3534; border: 1px solid #fa3534;" @click="gotodetail(item.id)" >查看详情</button>
	  </view>
	   <view class='status' v-if="item.status==9" style=" margin-left: calc(100% - 340rpx)">
	   <button style="color:#fa3534; border: 1px solid #fa3534;" @click="gotodetail(item.id)" >查看详情</button>
	    <button  style="color:#fa3534; border: 1px solid #fa3534;" @click="uploadfile(item.id)" >上传资料</button>
	  </view>
	  <view class='status' v-if="item.status==4" style=" margin-left: calc(100% - 340rpx)">
	  <button style="color:#fa3534; border: 1px solid #fa3534;" @click="gotodetail(item.id)" >查看详情</button>
