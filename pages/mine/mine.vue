      ...
      <view class="userinfo">
        ...
        <view class="userinfoView" :style="{ 'margin-top': statusbar }">
          ...
          <view class="userinfoTitle">
            ...
            <view
              class="headetitle"
              style="font-size: 28rpx; margin-top: 15rpx"
            ...
            <view
              class="bianji"
              @click="compile"
              v-if="userInfo.nickname != null && userInfo.nickname != ''"
              >编辑</view
            >
            <view class="bianji" @click="login" v-else>登录</view>
          </view>
        </view>
      </view>
      <view class="integralView" @click="integralClick">
        <image
          src="../../static/jifen-2.png"
          style="width: 60rpx; margin-left: 60rpx"
          mode="widthFix"
        ></image>
        <view class="integralViewTitle" style="color: #666666">我的积分</view>
        <view class="integralViewTitle" style="color: #f65535"
          >{{ userInfo.money }}分</view
        >
        <image
          class="image-jiantou"
          src="../../static/arrows.png"
          style="width: 30rpx; margin-right: 60rpx"
          mode="widthFix"
        ></image>
      </view>
    </view>
    <image
      src="../../static/kvip.jpeg"
      mode="widthFix"
      v-if="isvip != '1'"
      style="width: calc(100% - 60rpx); margin-left: 30rpx; margin-top: 30rpx"
      @click="buyvip"
    ></image>
    <view class="menuView">
      <div class="menuViewlist" @click="orderClick">
        <image
          src="../../static/myorder.png"
          style="width: 40%"
          mode="widthFix"
        ></image>
      ...
      <div class="menuViewlist" @click="payCode">
        <image
          ...
          mode="widthFix"
        ></image>
        <view class="menuViewtitle">收款二维码</view>
      </div>
      <div class="menuViewlist" @click="compile">
        <image
          src="../../static/gerenziliao.png"
          style="width: 40%"
          mode="widthFix"
        ></image>
...
export default {
  data() {
    return {
      ...
      isvip: "",
      login_flag: false,
      radioItems: [{ name: "微信零钱", value: "1", checked: true }],
      items: [
        ...
        success: function (res) {
          if (res.data.status == "1") {
            if (res.data.data.mobile == null) {
              ...
              that.isvip = res.data.data.isvip;
              that.phone =
                res.data.data.mobile.substring(0, 3) +
                "****" +
                res.data.data.mobile.substring(7);
            }
          } else {
          }
        },
    ...
    dunHuanAction: function (e) {
      ...
      if (that.userInfo.mobile == null) {
        ...
        if (e == 3) {
          ...
          uni.navigateTo({
            url:
              "../bingZhifubao/bingZhifubao?option=" + JSON.stringify(option),
          });
        }
        if (e == 4) {
          uni.navigateTo({
